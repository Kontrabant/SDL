...
SDL_Cursor *SDL_CreateAnimatedCursor(SDL_CursorFrameInfo *frames, int frame_count, int hot_x, int hot_y)
{
    ...
    for (int i = 0; i < frame_count; ++i) {
        // All cursor images should be the same size, and if the first cursor has alternate images, they all should.
        if (frames[i].surface->w != w || frames[i].surface->h != h) {
            SDL_SetError("All cursor image in an animated sequence must have the same dimensions.");
            goto cleanup;
        }
