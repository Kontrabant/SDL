...
static bool CheckToplevelWMCapability(SDL_Window *window, enum xdg_toplevel_wm_capabilities wm_capability)
{
    SDL_WindowData *wind = window->internal;
    ...
    if (wind->shell_surface_type == WAYLAND_SURFACE_LIBDECOR && wind->shell_surface.libdecor.frame) {
        ...
        if (wind->shell_surface_type == WAYLAND_SURFACE_XDG_TOPLEVEL && wind->shell_surface.xdg.toplevel.xdg_toplevel &&
            xdg_toplevel_get_version(wind->shell_surface.xdg.toplevel.xdg_toplevel) >= XDG_TOPLEVEL_WM_CAPABILITIES_SINCE_VERSION) {
        const int caps = wind->shell_surface.xdg.toplevel.wm_caps;
                                         ...
                                         struct wl_array *capabilities)
{
    ...
    wind->shell_surface.xdg.toplevel.wm_caps = 0;

    wl_array_for_each (wm_cap, capabilities) {
        switch (*wm_cap) {
        case XDG_TOPLEVEL_WM_CAPABILITIES_WINDOW_MENU:
            wind->shell_surface.xdg.toplevel.wm_caps |= WAYLAND_XDG_TOPLEVEL_WM_CAPS_WINDOW_MENU;
            break;
        case XDG_TOPLEVEL_WM_CAPABILITIES_MAXIMIZE:
            wind->shell_surface.xdg.toplevel.wm_caps |= WAYLAND_XDG_TOPLEVEL_WM_CAPS_MAXIMIZE;
            break;
        case XDG_TOPLEVEL_WM_CAPABILITIES_FULLSCREEN:
            wind->shell_surface.xdg.toplevel.wm_caps |= WAYLAND_XDG_TOPLEVEL_WM_CAPS_FULLSCREEN;
            break;
        case XDG_TOPLEVEL_WM_CAPABILITIES_MINIMIZE:
            wind->shell_surface.xdg.toplevel.wm_caps |= WAYLAND_XDG_TOPLEVEL_WM_CAPS_MINIMIZE;
            break;
        default:
            break;
        }
