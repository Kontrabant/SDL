...
void Wayland_ApplyZoneItemPosition(SDL_WindowData *wind)
{
    if (wind->current_ext_zone_v1) {
        ...
        int x = wind->requested.x - wind->borders.left - dst_display->logical.x;
        int y = wind->requested.y - wind->borders.top - dst_display->logical.y;
...
static bool Wayland_PixelPositionToLogical(SDL_WindowData *wind, int pixel_x, int pixel_y, int *logical_x, int *logical_y)
{
    SDL_VideoData *vid = wind->waylandData;
...
static bool Wayland_LogicalPositionToPixel(SDL_WindowData *wind, int logical_x, int logical_y, int *pixel_x, int *pixel_y)
{
    SDL_DisplayData *dst_display = Wayland_GetDisplayForWindowZone(wind, logical_x, logical_y, NULL);
...
static bool Wayland_PositionWindowInZone(SDL_WindowData *wind)
{
    ...
    if (!wind->scale_to_display) {
        ...
        if (!Wayland_PixelPositionToLogical(wind, wind->requested.x, wind->requested.y, &x, &y)) {
    ...
    SDL_DisplayData *dst_display = Wayland_GetDisplayForWindowZone(wind, x, y, NULL);
    if (dst_display) {
...
static void handle_ext_zone_item_v1_position(void *data, struct xx_zone_item_v1 *item, int32_t x, int32_t y)
{
    ...
    if (wind->scale_to_display) {
        int pixel_x, pixel_y;
        if (!Wayland_LogicalPositionToPixel(wind, w_x, w_y, &pixel_x, &pixel_y)) {
            return;
        }
