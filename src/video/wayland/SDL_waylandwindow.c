...
static void surface_frame_done(void *data, struct wl_callback *cb, uint32_t time)
{
    ...
    if (wind->shell_surface_status == WAYLAND_SHELL_SURFACE_STATUS_WAITING_FOR_FRAME) {
        ...
        if (wind->ext_zone_item_v1) {
            ...
            ext_zone_item_v1_set_position(wind->ext_zone_item_v1, wind->sdlwindow->x, wind->sdlwindow->y);
        }
...
static void ext_zone_item_v1_position_failed_handler(void *data, struct ext_zone_item_v1 *item)
{
    // NOP
}
        ...
        if (wind->is_fullscreen) {
            ...
            if (wind->floating) {
                ...
                if (dst_display) {
                    ...
                    x = SDL_clamp(x, -(window->w + 1), dst_display->zone_width ? dst_display->zone_width - 1 : x);
                    y = SDL_clamp(y, -(window->h + 1), dst_display->zone_height ? dst_display->zone_height - 1 : y);
