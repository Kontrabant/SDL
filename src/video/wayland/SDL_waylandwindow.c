...
static void ConfigureWindowGeometry(SDL_Window *window)
{
    ...
    if (window_size_changed) {
        /* XXX: This is a hack and only needed on the non-libdecor path when viewports
         *      aren't in use, in order to avoid a potential protocol violation if a
         *      buffer with an old size is committed.
         */
        if (!data->viewport && data->shell_surface_type != WAYLAND_SURFACE_LIBDECOR && data->shell_surface.xdg.surface) {
