...
static void ConfigureWindowGeometry(SDL_Window *window)
{
    ...
    if (window_size_changed) {
        /* libdecor does this internally on frame commits, so it's only needed for xdg surfaces, and
         * only when a viewport is not attached to the surface.
         */
        if (!data->draw_viewport && data->shell_surface_type != WAYLAND_SURFACE_LIBDECOR &&
