...
void Wayland_DestroyEventThread(Wayland_EventThreadContext *context)
{
    if (context->thread) {
        ...
        struct wl_display *display = context->wayland_data->display;
        struct wl_proxy *display_wrapper = WAYLAND_wl_proxy_create_wrapper(display);
        WAYLAND_wl_proxy_set_queue(display_wrapper, context->queue);
...
struct wl_proxy *Wayland_CreateEventThreadProxyWrapper(Wayland_EventThreadContext *context, void *obj)
{
