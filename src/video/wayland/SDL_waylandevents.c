...
static void Wayland_KeymapIterator(struct xkb_keymap *keymap, xkb_keycode_t key, void *data)
{
     ...
     *
     * Keys with no corresponding scancode will be dynamically added to the keymap with reserved scancodes
     * as they arrive.
     */
        ...
        for (xkb_level_index_t level = 0; level < num_levels; ++level) {
            if (WAYLAND_xkb_keymap_key_get_syms_by_level(seat->keyboard.xkb.keymap, key, layout, level, &syms) > 0) {
                ...
                for (size_t mask = 0; mask < num_masks; ++mask) {
                    const SDL_Keymod sdl_mod = (xkb_mod_masks[level] & seat->keyboard.xkb.shift_mask ? SDL_KMOD_SHIFT : 0) |
                                               (xkb_mod_masks[level] & seat->keyboard.xkb.ctrl_mask ? SDL_KMOD_CTRL : 0) |
                                               (xkb_mod_masks[level] & seat->keyboard.xkb.alt_mask ? SDL_KMOD_ALT : 0) |
                                               (xkb_mod_masks[level] & seat->keyboard.xkb.gui_mask ? SDL_KMOD_GUI : 0) |
                                               (xkb_mod_masks[level] & seat->keyboard.xkb.level3_mask ? SDL_KMOD_MODE : 0) |
                                               (xkb_mod_masks[level] & seat->keyboard.xkb.level5_mask ? SDL_KMOD_LEVEL5 : 0) |
                                               (xkb_mod_masks[level] & seat->keyboard.xkb.caps_mask ? SDL_KMOD_CAPS : 0);
                                ...
                                uint32_t state_w)
{
    ...
    if (scancode == SDL_SCANCODE_UNKNOWN) {
        SDL_Keycode keycode = (SDL_Keycode)SDL_KeySymToUcs4(syms[0]);
        if (keycode) {
