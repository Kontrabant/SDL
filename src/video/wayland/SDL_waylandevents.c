...
static xkb_keycode_t Wayland_GetScancodeFromTable(xkb_keycode_t key)
{
    SDL_Scancode scancode = SDL_SCANCODE_UNKNOWN;

    int entries;
    const SDL_Scancode *table = SDL_GetScancodeTable(SDL_SCANCODE_TABLE_XFREE86_2, &entries);
    if (key < entries) {
        scancode = table[key];
    }
    if (scancode == SDL_SCANCODE_UNKNOWN) {
        // The Linux table has entries beyond the limits of the XFree86_2 table.
        table = SDL_GetScancodeTable(SDL_SCANCODE_TABLE_LINUX, &entries);
        if (key < entries) {
            scancode = table[key];
        }
    }
...
static void Wayland_KeymapIterator(struct xkb_keymap *keymap, xkb_keycode_t key, void *data)
{
    ...
    if (!seat->keyboard.is_virtual) {
        scancode = Wayland_GetScancodeFromTable(key - 8);
    }
 ...
 */
static SDL_Scancode Wayland_GetScancodeForKey(SDL_WaylandSeat *seat, xkb_keycode_t key, const xkb_keysym_t **syms)
{
    SDL_Scancode scancode = SDL_SCANCODE_UNKNOWN;

    if (!seat->keyboard.is_virtual) {
        scancode = Wayland_GetScancodeFromTable(key);
