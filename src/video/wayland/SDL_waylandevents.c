...
static void Wayland_keymap_iter(struct xkb_keymap *keymap, xkb_keycode_t key, void *data)
{
    ...
    for (xkb_level_index_t i = 0; i < num_levels; ++i) {
        if (WAYLAND_xkb_keymap_key_get_syms_by_level(seat->keyboard.xkb.keymap, key, seat->keyboard.xkb.current_layout, i, &syms) > 0) {
            xkb_mod_mask_t masks[16];
            const size_t num_masks = WAYLAND_xkb_keymap_key_get_mods_for_level(seat->keyboard.xkb.keymap, key, seat->keyboard.xkb.current_layout, i, masks, SDL_arraysize(masks));
            for (size_t j = 0; j < num_masks; ++j) {
                const SDL_Keymod sdl_mod = (masks[j] & seat->keyboard.xkb.shift_mask ? SDL_KMOD_SHIFT : 0) |
                                           (masks[j] & seat->keyboard.xkb.ctrl_mask ? SDL_KMOD_CTRL : 0) |
                                           (masks[j] & seat->keyboard.xkb.alt_mask ? SDL_KMOD_ALT : 0) |
                                           (masks[j] & seat->keyboard.xkb.gui_mask ? SDL_KMOD_GUI : 0) |
                                           (masks[j] & seat->keyboard.xkb.level3_mask ? SDL_KMOD_MODE : 0) |
                                           (masks[j] & seat->keyboard.xkb.level5_mask ? SDL_KMOD_LEVEL5 : 0) |
                                           (masks[j] & seat->keyboard.xkb.caps_mask ? SDL_KMOD_CAPS : 0);
