                                           ...
                                           uint32_t identity)
{
     ...
     * Otherwise, it will inherit the color info queue from the image description object.
     */
    if (state->deferred_event_processing) {
        SDL_VideoData *vid = SDL_GetVideoDevice()->internal;
        state->queue = Wayland_DisplayCreateQueue(vid->display, "SDL Color Management Queue");

        struct wl_proxy *desc_wrapper = WAYLAND_wl_proxy_create_wrapper(state->wp_image_description);
        WAYLAND_wl_proxy_set_queue(desc_wrapper, state->queue);
        state->wp_image_description_info = wp_image_description_v1_get_information((struct wp_image_description_v1 *)desc_wrapper);
        WAYLAND_wl_proxy_wrapper_destroy(desc_wrapper);
    } else {
...
void Wayland_GetColorInfoForWindow(SDL_WindowData *window_data, bool defer_event_processing)
{
    ...
    if (state) {
        ...
        if (!defer_event_processing) {
            ...
            struct wl_proxy *sf_wrapper = WAYLAND_wl_proxy_create_wrapper(window_data->wp_color_management_surface_feedback);
            WAYLAND_wl_proxy_set_queue(sf_wrapper, state->queue);
            state->wp_image_description = wp_color_management_surface_feedback_v1_get_preferred((struct wp_color_management_surface_feedback_v1 *)sf_wrapper);
            WAYLAND_wl_proxy_wrapper_destroy(sf_wrapper);
        } else {
...
void Wayland_GetColorInfoForOutput(SDL_DisplayData *display_data, bool defer_event_processing)
{
    ...
    if (state) {
        ...
        if (!defer_event_processing) {
            ...
            struct wl_proxy *of_wrapper = WAYLAND_wl_proxy_create_wrapper(display_data->wp_color_management_output);
            WAYLAND_wl_proxy_set_queue(of_wrapper, state->queue);
            state->wp_image_description = wp_color_management_output_v1_get_image_description((struct wp_color_management_output_v1 *)of_wrapper);
            WAYLAND_wl_proxy_wrapper_destroy(of_wrapper);
        } else {
