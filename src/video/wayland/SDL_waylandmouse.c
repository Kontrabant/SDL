...
static void handle_cursor_animation_exit(void *data, struct wl_callback *wl_callback, uint32_t callback_data)
{
...
static const struct wl_callback_listener cursor_thread_exit_listener = {
    handle_cursor_animation_exit
};

static SDLCALL int Wayland_CursorAnimationThreadFunc(void *data)
{
    ...
    while (!cursor_thread_context.should_exit) {
        ...
        if (WAYLAND_wl_display_prepare_read_queue(display, cursor_thread_context.queue) == 0) {
            ...
            if (ret == -1 && errno == EAGAIN) {
                ret = SDL_IOReady(display_fd, SDL_IOR_WRITE, SDL_MS_TO_NS(10));
                if (ret > 0) {
                    ret = WAYLAND_wl_display_flush(display);
                }
            }

            // Exit the thread on a broken pipe.
            if (ret < 0 && errno == EPIPE) {
                WAYLAND_wl_display_cancel_read(display);
                return -1;
            }

            ret = SDL_IOReady(display_fd, SDL_IOR_READ, -1);
        ...
        WAYLAND_wl_display_dispatch_queue_pending(display, cursor_thread_context.queue);
...
static bool Wayland_StartCursorAnimationThread(SDL_VideoData *data)
{
    if (!cursor_thread_context.thread) {
        ...
        cursor_thread_context.thread = SDL_CreateThread(Wayland_CursorAnimationThreadFunc, "wl_cursor_animation", data->display);
        if (!cursor_thread_context.thread) {
            goto cleanup;
        }
...
static void Wayland_DestroyCursorAnimationThread(SDL_VideoData *data)
{
    if (cursor_thread_context.thread) {
        ...
        if (ret == -1 && errno == EAGAIN) {
            ret = SDL_IOReady(WAYLAND_wl_display_get_fd(data->display), SDL_IOR_WRITE, SDL_MS_TO_NS(100));
            if (ret >= 0) {
...
void Wayland_InitMouse(SDL_VideoData *data)
{
    ...
    if (!Wayland_StartCursorAnimationThread(data)) {
...
{
    Wayland_DestroyCursorAnimationThread(data);
