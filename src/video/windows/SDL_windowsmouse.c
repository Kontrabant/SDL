...
/* Windows doesn't have the ability to easily create animated cursors programmatically,
 * so we build a cursor resource file in memory and load it.
 */
static HCURSOR WIN_CreateAnimatedCursorInternal(SDL_CursorFrameInfo *frames, int frame_count, int hot_x, int hot_y, float scale)
{
    ...
    // Windows cursors have a hard size limit of 256x256.
