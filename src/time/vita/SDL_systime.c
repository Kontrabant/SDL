...
void SDL_UpdateTZNames()
{
    ...
    if (sceAppUtilSystemParamGetInt(SCE_SYSTEM_PARAM_ID_TIME_ZONE, &tzOffset) == 0) {
...
int SDL_GetRealtimeClock(SDL_TimeSpec *ts)
{
    ...
    if (!ret) {
        ...
        ts->seconds = (Sint64)(sceTicks.tick / res) - UNIX_EPOCH_OFFSET_SEC;
        ts->nanoseconds = (int)(sceTicks.tick / res) * (SDL_NS_PER_SECOND / res);
        return 0;
    }
...
int SDL_TimeSpecToUTCDateTime(const SDL_TimeSpec *ts, SDL_DateTime *dt)
{
    ...
    const unsigned int res = sceRtcGetTickResolution();
    sceTicks.tick = ((ts->seconds + UNIX_EPOCH_OFFSET_SEC) * res) + (ts->nanoseconds * (SDL_NS_PER_SECOND / res));

    const int ret = sceRtcSetTick(&t, &sceTicks);
    if (!ret) {
        dt->year = t.year;
        dt->month = t.month;
        dt->day = t.day;
...
int SDL_TimeSpecToLocalDateTime(const SDL_TimeSpec *ts, SDL_DateTime *dt)
{
    ...
    int ret = sceRtcConvertUtcToLocalTime(&sceTicks, &sceLocalTicks);
    if (!ret) {
        ...
        if (!ret) {
            ...
            dt->second = t.second;
            dt->nanosecond = t.microsecond * 1000;
            dt->utc_offset = (int)(((Sint64)sceTicks.tick - (Sint64)sceLocalTicks.tick) / (Sint64)res);

            int dst = 0;
            if (sceAppUtilSystemParamGetInt(SCE_SYSTEM_PARAM_ID_DAYLIGHT_SAVINGS, &dst) == 0) {
