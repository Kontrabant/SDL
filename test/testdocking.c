...
static void DrawDockable(SDL_Renderer *renderer, float x, float y)
{
    ...
    const SDL_FRect rect = { x, y, DOCK_WIN_WIDTH, DOCK_WIN_HEIGHT };
...
SDL_AppResult SDL_AppEvent(void *appstate, SDL_Event *event)
{
    ...
    case SDL_EVENT_DROP_WINDOW:
    {
        ...
        const int x = SDL_lroundf(event->drop.x) - SDL_GetNumberProperty(props, SDL_PROP_WINDOW_DRAG_OFFSET_X_NUMBER, 0);
        const int y = SDL_lroundf(event->drop.y) - SDL_GetNumberProperty(props, SDL_PROP_WINDOW_DRAG_OFFSET_Y_NUMBER, 0);
