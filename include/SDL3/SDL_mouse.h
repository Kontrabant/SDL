 ...
 *
 * Animated cursors are composed from a sequential array of frames, specified
 * as surfaces and durations in an array of SDL_CursorFrameInfo structs.
 * The hotspot coordinates apply to all frames, and all frames must have the same
 * dimensions.
 *
 * Frame durations are specified in milliseconds. A duration of 0 implies an
 * infinite frame time. To create a one-shot animation, set the duration of
 * the last frame in the sequence to 0.
 *
